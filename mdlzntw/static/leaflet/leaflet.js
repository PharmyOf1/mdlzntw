...
//Bakery Circles
function point_gen(coords,size=50000) {
            var long = coords[0];
            var lat = coords[1];
            var circle = L.circle([long, lat], size, {
                                color: 'red',
                                fillColor: '#FFF',
                                title: 'Hover Text',
                                fillOpacity: 0.5
...
//Create Circles for bakeries
var points = [
                [33.715700,-84.422823, 'Atlanta'],
                [45.581046, -122.665531, 'Portland'],
                [37.497038, -77.350683, 'Richmond'],
                [40.948303, -74.133321, 'Fairlawn'],
                [41.759438, -87.705189, 'Chicago'],
                [41.771343, -88.188280, 'Naperville'],
                [22.627610, -101.716498, 'Salinas'],
                [25.698367, -100.227740, 'Monterrey'],
                [43.713080, -79.307862, 'East York'],
                [43.774151, -79.267722, 'Scarborough'],
                [45.564428, -73.550000, 'Montreal']
            ];

//Generate Points
function set_points(map_name,zoomL) {
            bakery_locs=[];
            if (zoomL==4) {zoomL=.8;}; 
...
function generate_map() {
        ...
        bakeries = set_points(map,map.getZoom());
           
    function add_polys() {
        var Atlanta = bakeries[0];
            Atlanta.addTo(map);
                        global_points.push(Atlanta);
        var Portland = bakeries[1];
            Portland.addTo(map);
                        global_points.push(Atlanta);
        var Richmond = bakeries[2];
            Richmond.addTo(map);
                        global_points.push(Atlanta);
        var Fairlawn = bakeries[3];
            Fairlawn.addTo(map);
                        global_points.push(Atlanta);
        var Chicago = bakeries[4];
            Chicago.addTo(map);
                        global_points.push(Atlanta);
        var Naperville = bakeries[5];
            Naperville.addTo(map);
                        global_points.push(Atlanta);
        var Salinas = bakeries[6];
            Salinas.addTo(map);
                        global_points.push(Atlanta);
        var Monterrey = bakeries[7];
            Monterrey.addTo(map);
                        global_points.push(Atlanta);
        var EastYork = bakeries[8];
            EastYork.addTo(map);
                        global_points.push(Atlanta);
        var Scarborough = bakeries[9];
            Scarborough.addTo(map);
                        global_points.push(Atlanta);
        var Montreal = bakeries[10];
            Montreal.addTo(map);
                        global_points.push(Atlanta);
    };
        add_polys();
        ...
        //Controls everything when user zooms ins and out
        map.on('zoomend', function() {

                                //Removes all polygons on zoom
                                var layers = 0;
                                for(i in map._layers) {
                                                        if (map._layers[i]._path != undefined) {layers = layers+i;map.removeLayer(map._layers[i]);}
                                };

                                //Resets all polygons on zoom
                                bakeries = set_points(map,map.getZoom());
                                global_points =[]
                                add_polys();
        
                             
                        //Sets zoom bound at 4 to ensure we stay in NA
                        if (map.getZoom()==4) {map.setMaxBounds(bounds);}
